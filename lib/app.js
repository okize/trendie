...
var percentChange = function (initial, final) {
  ...
  var change,
      ...
      checkIfNaN = function (num) {
        return num !== num;
      },
      checkIfInfinity = function (num) {
  ...
  if (checkIfNaN(change)) {
    change = 0;
  }

  if (checkIfInfinity(change)) {
    change = 1;
  }
...
exports.getTrends = function (profileId, username, password, jsonOut) {
  ...
  var
    ...
    dateOneMonthAgo = moment(dateToday).subtract(28, 'days').format(dateFormat),
    dateSixMonthsAgo = moment(dateToday).subtract(183, 'days').format(dateFormat),
    dateSevenMonthsAgo = moment(dateSixMonthsAgo).subtract(28, 'days').format(dateFormat),
    dateYearAgo = moment(dateToday).subtract(365, 'days').format(dateFormat),
    dateYearAndMonthAgo = moment(dateYearAgo).subtract(28, 'days').format(dateFormat),
    gaRequestOpts = {
